...
class Gamepad:
    MAX_TRIGGER = 1023  # LATER: configure this
    ...
    async def read_gamepad_input(self):  # asyncronus read-out of events
        max_abs_joystick_left_x = 0xFFFF / 2  # LATER: configure this
        uncertainty_joystick_left_x = 2500  # HACK: investigate uncertainty
        ...
        async for event in self.device_file.async_read_loop():
            ...
            if event.type == 3:  # type is analog trigger or joystick
                if event.code == 1:  # left joystick y-axis
                    ...
                    self.trigger_right = event.value / gamepad.MAX_TRIGGER
                elif event.code == 2:  # left trigger
                    self.trigger_left = event.value / gamepad.MAX_TRIGGER
